"""Unit tests for the JSON scalar query helpers in supabase_db_helpers.py.
...
# ===================================================================
# parse_json_scalar — structure and type validation
# ===================================================================
...
class TestParseJsonScalar:
    ...
    def test_valid_bool_true(self):
        ...
        assert parse_json_scalar(data, "result", bool, "boolean") is True

    def test_valid_bool_false(self):
        data = json.dumps([{"result": False}])
        assert parse_json_scalar(data, "result", bool, "boolean") is False

    def test_valid_int(self):
        """2. Valid JSON with integer must return the integer."""
        data = json.dumps([{"count": 42}])
        assert (
            parse_json_scalar(data, "count", int, "integer", reject_bool=True) == 42
        )

    def test_valid_int_zero(self):
        data = json.dumps([{"count": 0}])
        assert (
            parse_json_scalar(data, "count", int, "integer", reject_bool=True) == 0
        )
    ...
    def test_bool_where_int_rejected(self):
        ...
        with pytest.raises(AssertionError, match="expected an integer, got boolean"):
            parse_json_scalar(data, "count", int, "integer", reject_bool=True)
    ...
    def test_sanitized_message_no_raw_output(self):
        ...
        for raw in raw_payloads:
            with pytest.raises(AssertionError) as exc:
                parse_json_scalar(raw, "result", bool, "boolean")
            msg = str(exc.value)
...
# ===================================================================
# query_scalar_bool — integrated with mocked run_supabase_op
# ===================================================================


class TestQueryScalarBool:
    """Tests for query_scalar_bool with a mocked ``run_supabase_op``."""

    @patch("backend.tests.supabase_db_helpers.run_supabase_op")
    def test_returns_true(self, mock_run_op):
        ...
        assert query_scalar_bool("SELECT true AS result", "result") is True
    ...
    def test_returns_false(self, mock_run_op):
        ...
        assert query_scalar_bool("SELECT false AS result", "result") is False
    ...
    def test_uses_supported_json_cli_contract(self, mock_run_op):
        ...
        mock_run_op.return_value = _fake_result('[{"result": true}]')
        query_scalar_bool("SELECT true AS result", "result")
    ...
    def test_invalid_json_sanitized(self, mock_run_op):
        ...
        with pytest.raises(AssertionError) as exc:
            query_scalar_bool("SELECT 1", "result")
    ...
    def test_null_sanitized(self, mock_run_op):
        ...
        with pytest.raises(AssertionError) as exc:
            query_scalar_bool("SELECT NULL AS result", "result")
        assert "expected a boolean value" in str(exc.value)


# ===================================================================
# query_scalar_int — integrated with mocked run_supabase_op
# ===================================================================


class TestQueryScalarInt:
    """Tests for query_scalar_int with a mocked ``run_supabase_op``."""
    ...
    def test_returns_int(self, mock_run_op):
        ...
        assert query_scalar_int("SELECT 42 AS count", "count") == 42

    @patch("backend.tests.supabase_db_helpers.run_supabase_op")
    def test_uses_supported_json_cli_contract(self, mock_run_op):
        """Must call run_supabase_op with --agent=no --output json (not --output-format)."""
        mock_run_op.return_value = _fake_result('[{"count": 42}]')
        query_scalar_int("SELECT 42 AS count", "count")
        mock_run_op.assert_called_once_with(
            "database_integration_query",
            [
                "db",
    ...
    def test_returns_zero(self, mock_run_op):
        ...
        assert query_scalar_int("SELECT 0 AS count", "count") == 0
    ...
    def test_bool_rejected(self, mock_run_op):
        ...
        with pytest.raises(AssertionError, match="expected an integer, got boolean"):
            query_scalar_int("SELECT true AS count", "count")
    ...
    def test_negative_rejected(self, mock_run_op):
        ...
        with pytest.raises(AssertionError, match="expected a non-negative integer"):
            query_scalar_int("SELECT -1 AS count", "count")
    ...
    def test_missing_key_sanitized(self, mock_run_op):
        ...
        with pytest.raises(AssertionError) as exc:
            query_scalar_int("SELECT 1 AS other", "count")
